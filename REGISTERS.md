...
- `SPC` - Program Counter
    - Part of *Core*
- `SFLAGS` - Flag Register
    - Contains NZCV flags, i.e. Negative, Zero, Carry, Overflow flags
    - Format - `| 32 . . . . . . . UNUSED BITS . . . . . . . 4 | V | C | N | Z |`
    - Part of *Core*, used heavily by *CoreX*
- `SFPSR` - Floating-Point Special Purpose register
    - Contains Floating point specific Flags
    - Flags Include -
        - DZ - Division by Zero
        - OF - Overflow (result → ±Inf)
        - UF - Underflow (result → 0), when precision is too high
        - NX - Inexact (If floating point result is not exact from roundgin)
        - NV - Invalid Floating Point Operation (NaN or Inf input)
